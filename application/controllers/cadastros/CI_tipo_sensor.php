<?php
class CI_tipo_sensor extends CI_controller {
	...
	function pesquisa($nr_pagina=20 ){
		...
		$this->dados["total"] = $this->M_tipo_sensor->numeroLinhasTotais();
	...
	function excluir($id=""){
		if ($id==""){
			...
			if(isset($_POST["item"])) {
				$this->M_tipo_sensor->setTipoSensorId($_POST["item"]);	
				$this->M_tipo_sensor->excluir();
			}
		}
		else{
			$this->M_tipo_sensor->setTipoSensorId($id);	
			$this->M_tipo_sensor->excluir();
		}
   ...
   function editar($valor = "") {
		if(isset($_POST["item"])) {
			$this->dados["registro"] = $this->M_tipo_sensor->selecionar($_POST["item"]);
		} else if ($valor != "") {
			$this->dados["registro"] = $this->M_tipo_sensor->selecionar($valor);
		}
		$this->cadastro();
	}
