...
class M_gateway extends CI_Model{
    ...
    private $gateway_status;
        ...
        function pesquisar($select='', $where=array(), $limit=10, $offset=0, $ordem='asc') {
            ...
            $this->db->join('fabricante as f', 'gateway.fabricante_id = f.fabricante_id', 'left');
            $this->db->join('servidorborda as b', 'gateway.servidorborda_id = b.servidorborda_id', 'left');
        ...
        function getBySBid($codigo) {
            ...
            return $this->db->get('gateway')->result_array();
        }
        ...
        function altStatus() {
			...
			$this->db->update('gateway', $arrayCampos, array("gateway_id"=>$this->gateway_id));
	        return "alt";
        }
		...
		function salvar() {
            $arrayCampos  = array(
                ...
                "uid" 		=> $this->gateway_uid,
                "status" 		=> $this->gateway_status
            );
			if ($this->gateway_id == ""){
	            $this->db->insert('gateway', $arrayCampos);
		        return "inc";
			}
        	else{
	            $this->db->update('gateway', $arrayCampos, array("gateway_id"=>$this->gateway_id));
		...
		function salvaGateway() {
            ...
            if ($this->checkByUID($this->gateway_uid) > 0){
            	$this->db->update('gateway', $arrayCampos, array("uid"=>$this->gateway_uid));
            	$this->db->where(array("uid"=>$this->gateway_uid));
		...
		function excluir() {
            ...
            if ($this->db->delete('gateway', $arrayCampos))
	            return true;
	        else
	        	return false;
		}
...
// Getters and Setters 
		...
		public function getGatewayUID() {
		    if($this->gateway_uid === NULL) {
        		$this->gateway_uid = new GatewayUID;
    		}			
