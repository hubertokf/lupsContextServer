<?php
	class M_fabricante extends CI_Model{
		...
		function salvar() {
			...
			if ($this->fabricante_id == ""){
	            $this->db->insert('fabricante', $arrayCampos);
		        return "inc";
			}
        	else{
	            $this->db->update('fabricante', $arrayCampos, array("fabricante_id"=>$this->fabricante_id));
		        return "alt";
        	}
		}
		...
		function excluir() {
            ...
            if ($this->db->delete('fabricante', $arrayCampos))
	            return true;
	        else
	        	return false;
		}
